...
async def execute_command_and_generate_image(
    command: str, ctx: AgentCtx, config, max_timeout: int = 30
) -> str:
    """
    执行shell命令并将结果转换为图片
    ...
    try:
        ...
        try:
            ...
            return await _create_error_image(f"命令执行超时 ({max_timeout}秒)", ctx)
        except Exception as e:
            logger.error(f"命令执行失败: {e}")
            return await _create_error_image(f"命令执行失败: {str(e)}", ctx)
        ...
        # 生成图片
        image_path = await _create_command_output_image(full_output, ctx)
        logger.info(f"成功执行命令并生成图片: {command}")
        return image_path
        
    except Exception as e:
        logger.exception(f"执行命令时发生未知错误: {e}")
        return await _create_error_image(f"未知错误: {str(e)}", ctx)
...
async def _create_error_image(error_message: str, ctx: AgentCtx) -> str:
    """
    创建错误信息图片
    ...
    """
    try:
        return await _create_simple_error_image(error_message, ctx)
    except Exception as e:
...
async def _create_simple_error_image(error_message: str, ctx: AgentCtx) -> str:
    """
    创建简单的错误图片
    ...
    # 保存图片
    output_path = ctx.fs.shared_path / "error_output.png"
    image.save(output_path, "PNG")
